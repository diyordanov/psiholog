...
- **Cloudflare Pages** [4] — статичен хостинг за компилирания SPA bundle (HTML, JavaScript, CSS, шрифтове, PDF.js worker). Автоматичен deploy при всеки push към `main` клона на GitHub хранилището; глобален CDN осигурява ниска латентност при зареждане навсякъде по света.
- **Supabase** [31] **Postgres** — релационна база данни за всички метаданни на приложението: потребителски профили, сертификатни ключове (в криптиран вид), документи, подписи, многоучастникови заявки. Защитена изцяло чрез Row Level Security (RLS) политики — виж 5.4.
...
- **Supabase Edge Functions (Deno runtime** [6]**)** — единствената сървърна операция с достъп до чувствителна тайна: издаването на X.509 сертификати от частния ключ на вътрешния Root CA (mini-CA, виж 5.4). Всичко останало в приложението работи без сървърна логика.
...
Технически най-нетривиалната част от този поток е **редът**, в който се изчисляват двата подписа. ECDSA P-256 подписът се вгражда в `/Contents` под формата на CMS (Cryptographic Message Syntax) [15] структура — стандартният контейнер, който Adobe Acrobat разпознава. И ECDSA, и ML-DSA-65 подписват дайджест на документа, но всеки алгоритъм подписва различен дайджест поради архитектурно ограничение: PDF стандартът изисква стойността в `/ByteRange` да изключва **точно** байтовете на `/Contents` placeholder-а, нищо повече — Adobe Acrobat отхвърля подпис, при който изключеният диапазон е по-широк ("SigDict /Contents illegal data"). Ако ML-DSA-65 подписът трябваше да живее в същия изключен диапазон като ECDSA (за да остане "защитен" от манипулация), диапазонът би станал по-широк от `/Contents` и Adobe валидацията би се провалила.
...
[28] PostgreSQL Global Development Group. *Row Security Policies* (Политики за защита на редове). PostgreSQL 16 Documentation, Chapter 5.9. https://www.postgresql.org/docs/current/ddl-rowsecurity.html. 2024.
